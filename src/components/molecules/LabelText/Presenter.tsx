import { CSSProperties, memo, ReactNode, VFC } from "react";
...
export const Presenter: VFC<Props> = memo(
  ({ children, className, color = "purple", isClickable = false, size = "md", style }) => {
    const baseClass =
      "inline-flex items-center border rounded transition ease-out duration-200 font-bold leading-none whitespace-pre-wrap";
    ...
    return (
      <div
        className={`${baseClass} ${cursorClass} ${sizeClass} ${fontSize} ${backgroundColorClass} ${backgroundColorHoverClass} ${borderColorClass} ${textColor} ${borderColorHoverClass} ${className}`}
        style={style}
      >
        {children}
      </div>
